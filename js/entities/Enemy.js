...
(function (global) {
    ...
    global.Enemy = function (map, direction, speed) {
        var image = Resources.get(AppResources.enemy.bug);
        global.AutomatedSpirit.call(this, map, image, direction, speed);
        var boundaryRectangle = PositionUtility.getRectangle(-200, 0, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY);
    ...
    global.Enemy.prototype.isCollided = function (player) {
        ...
        return PositionUtility.areWithinTolerance(playerPosition.x, this.position.x, this.map.dimension.blockSize.width * 0.7) &&
            PositionUtility.areWithinTolerance(playerPosition.y, this.position.y, this.map.dimension.blockSize.height * 0.2);
    };
